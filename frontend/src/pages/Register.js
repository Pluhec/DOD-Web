...
const Register = () => {
  ...
  const formik = useFormik({
    initialValues: {
      username: '', // changed from nickname to username
      email: '',
      password: ''
    },
    validationSchema: Yup.object({
      username: Yup.string().min(6, 'Nickname must be at least 6 characters long').required('Username is required'),
    ...
    onSubmit: async (values) => {
      try {
        const response = await fetch('http://localhost:3000/api/auth/register', {
          ...
          body: JSON.stringify({
            nickname: values.username, // map username to nickname
            email: values.email,
    ...
    <div className='flex flex-col items-center justify-center h-screen bg-background'>
      ...
      <div className='bg-secondaryBackground w-full max-w-sm p-8 rounded-lg shadow-lg'>
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <label htmlFor="email" className="block text-textPrimary mb-2">Email address</label>
          <input
            className='w-full px-4 py-2 mb-4 border border-accent rounded-lg focus:outline-none focus:ring-2 focus:ring-accent bg-secondaryBackground text-textSecondary'
            type="email"
            id="email"
        ...
        <div className="text-center mt-6 flex flex-col justify-center">
          <p className="text-textSecondary font-thin text-s">Already a member?</p> 
          <Link to="/login" className="text-textPrimary font-bold hover:underline">Login</Link>
        </div> 
      </div>
    </div>
  );
